...
public class Main extends JPanel implements KeyListener {
    ...
    public void keyPressed(KeyEvent e) {
        ...
        System.out.println(mosesPoint.x +" , "+ mosesPoint.y);
        switch (e.getKeyCode()){
            case KeyEvent.VK_UP:
                if(mosesPoint.y > 1){
                ...
                for(int i = mosesPoint.y; i > 0; i--){
                    for(Sprite s : gameView.getElements()){
                        if(s.getRelativePosition()!= null && s.getRelativePosition().x == mosesPoint.x && s.getRelativePosition().y == i){
                            ...
                            if(s instanceof Anubis || s instanceof Pharaoah || s instanceof Bug || s instanceof Frog){
                                s.setNullPosition();
                                repaint();
                                return;
                            }
